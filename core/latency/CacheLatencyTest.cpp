...
private:
  struct Node {
    Node *next;
    char padding[56]; // To fill a typical cache line 8 bytes struct + 56 bytes
                      // padding
  };
   ...
   * of nodes where each node points to another in random order, forming a
   * cycle. The randomization ensures cache misses are distributed throughout
   * memory.
   *
...
public:
   ...
   *
   * This method creates a circular linked list of the specified size with
   * randomized pointer connections, then measures the average time (in CPU
   * cycles) required to follow each pointer. The randomization ensures cache
   * misses are distributed throughout memory, providing accurate latency
   * measurements.
   *
   * @param size_bytes Memory size for the pointer chain (affects cache level
   * tested)
   * @param chase_count Number of pointer dereferences to perform (default: 10M)
   * @return LatencyResult Structure containing latency measurements and
   * metadata
